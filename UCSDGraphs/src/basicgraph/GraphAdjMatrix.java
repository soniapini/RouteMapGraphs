...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		int[][] twoHopsMat = new int[getNumVertices()][getNumVertices()];
		for (int row=0; row<getNumVertices(); row++) {
			for (int col=0; col<getNumVertices(); col++) {
				twoHopsMat[row][col] = 0;
				for (int vert=0; vert<getNumVertices(); vert++) {
					twoHopsMat[row][col] += adjMatrix[row][vert] * adjMatrix[vert][col];
				}
			}
		}
		...
		for (int vert=0; vert<getNumVertices(); vert++) {
			for(int i=0; i<twoHopsMat[v][vert]; i++) {
				twoHops.add(vert);
			}
		}
		return twoHops;
