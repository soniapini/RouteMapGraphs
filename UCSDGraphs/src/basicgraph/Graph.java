...
public abstract class Graph {
	...
	public List<Integer> degreeSequence() {
		...
		List<Integer> allDegrees = new ArrayList<>(numVertices);
		for (int vertexIdx=0;  vertexIdx < numVertices; vertexIdx++) {
			int degree = getNeighbors(vertexIdx).size() + getInNeighbors(vertexIdx).size();
			allDegrees.add(degree);
		}
		
		Collections.sort(allDegrees, new Comparator<Integer>() {
